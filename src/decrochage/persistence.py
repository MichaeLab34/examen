...
def _build_silver_frames(
    raw_df: pd.DataFrame, catalogue_df: pd.DataFrame
) -> tuple[pd.DataFrame, pd.DataFrame]:
    silver_students = clean_raw(raw_df)
...
def _build_gold_frame(
    ...
    gold = F.enrich_with_catalogue(silver_students, silver_catalogue)
    gold = F.add_engineered_features(gold)
    feature_cols = F.scoring_feature_columns(gold)
...
def persist_medallion_layers(
    ...
    *,
    ...
    batch = _create_batch(
        ...
        source_name=source_name,
        source_uri=source_uri,
        metadata={"architecture": "medallion", "layers": ["bronze", "silver", "gold"]},
    )

    silver_students, silver_catalogue = _build_silver_frames(raw_df, catalogue_df)
...
def persist_predictions(
    ...
    for idx, scored_row in scored_df.reset_index(drop=True).iterrows():
        raw_payload = raw_records[idx] if idx < len(raw_records) else {}
